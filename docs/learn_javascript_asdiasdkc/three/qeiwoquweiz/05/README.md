...
一个 [简单的请求](https://fetch.spec.whatwg.org/#terminology) 是指满足以下两个条件的请求：
...
例如，如果我们从 `https://relieved-hai.github.io/page` 请求 `https://anywhere.com/request`，请求的 header 将会如下：
...
服务器可以检查 `Origin`，如果同意接受这样的请求，就会在响应中添加一个特殊的 header `Access-Control-Allow-Origin`。该 header 包含了允许的源（在我们的示例中是 `https://relieved-hai.github.io`），或者一个星号 `*`。然后响应成功，否则报错。
...
- `Access-Control-Allow-Origin` 必须为 `*` 或进行请求的源（例如 `https://relieved-hai.github.io`）才能允许此请求。
- `Access-Control-Allow-Methods` 必须具有允许的方法。
- `Access-Control-Allow-Headers` 必须具有一个允许的 header 列表。
- 另外，header `Access-Control-Max-Age` 可以指定缓存此权限的秒数。因此，浏览器不是必须为满足给定权限的后续请求发送预检。

![](./images/xhr-preflight.svg)
