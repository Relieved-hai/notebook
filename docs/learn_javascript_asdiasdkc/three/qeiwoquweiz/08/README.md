...
这些话听起来熟悉吗？如果是，那么请继续阅读下面的 `XMLHttpRequest` 相关内容吧。如果还不是很熟悉的话，那么请先阅读 [Fetch](../../qeiwoquweiz/01/README.md) 一章的内容。
...
- 2. 初始化它，通常就在 `new XMLHttpRequest` 之后：
  ...
  - `URL` —— 要请求的 URL，通常是一个字符串，也可以是 [URL](../../qeiwoquweiz/07/README.md) 对象。
...
为了向 URL 添加像 `?name=value` 这样的参数，并确保正确的编码，我们可以使用 [URL](../../qeiwoquweiz/07/README.md) 对象：
...
- **`""`**（默认）—— 响应格式为字符串，
- **`"text"`** —— 响应格式为字符串，
- **`"arraybuffer"`** —— 响应格式为 ArrayBuffer（对于二进制数据，请参见 [ArrayBuffer，二进制数组](../../yhntgbcdexsw/01/README.md) ），
- **`"blob"`** —— 响应格式为 `Blob`（对于二进制数据，请参见 [Blob](../../yhntgbcdexsw/03/README.md) ），
...
header 之间的换行符始终为 `"\r\n"`（不依赖于操作系统），所以我们可以很容易地将其拆分为单独的header。name 和 value 之间总是以冒号后跟一个空格 `": "` 分隔。这是标准格式。
...
要建立一个 POST 请求，我们可以使用内建的 [FormData](https://developer.mozilla.org/zh-CN/docs/Web/API/FormData) 对象。
...
`XMLHttpRequest` 可以使用和 [fetch](../../qeiwoquweiz/05/README.md) 相同的 CORS 策略进行跨源请求。
...
有关跨源 header 的详细信息，请见 [Fetch：跨源请求](../../qeiwoquweiz/05/README.md) 。
...
实际上还有很多事件，在 [现代规范](https://www.w3.org/TR/XMLHttpRequest/#events) 中有详细列表（按生命周期排序）：
