...
- **`response.formData()`** —— 以 `FormData` 对象（在 [下一章](../../qeiwoquweiz/02/README.md) 有解释）的形式返回 response，
- **`response.blob()`** —— 以 [Blob](../../yhntgbcdexsw/03/README.md)（具有类型的二进制数据）形式返回 response，
- **`response.arrayBuffer()`** —— 以 [ArrayBuffer](../../yhntgbcdexsw/01/README.md)（低级别的二进制数据）形式返回 response，
...
```js {4}
let url = 'https://relieved-hai.github.io/notebook/learn_javascript_asdiasdkc/three/qeiwoquweiz/01/index.json';
let response = await fetch(url);
...
```js
fetch('https://relieved-hai.github.io/notebook/learn_javascript_asdiasdkc/three/qeiwoquweiz/01/index.json')
...
```js
let response = await fetch('https://relieved-hai.github.io/notebook/learn_javascript_asdiasdkc/three/qeiwoquweiz/01/index.json');
...
作为一个读取为二进制格式的演示示例，让我们 fetch 并显示一张 [“fetch” 规范](https://fetch.spec.whatwg.org/) 中的图片（Blob 操作的有关内容请见 [Blob](../../yhntgbcdexsw/03/README.md) ）：

```js {3}
let response = await fetch('/learn_javascript_asdiasdkc/three/qeiwoquweiz/01/logo-fetch.svg');
...
- **`body`** —— request body，其中之一：
  ...
  - [URLSearchParams](../../qeiwoquweiz/07/README.md) ，以 `x-www-form-urlencoded` 编码形式发送数据，很少使用。
...
- **`response.blob()`** —— 以 [Blob](../../yhntgbcdexsw/03/README.md)（具有类型的二进制数据）形式返回 response，
- **`response.arrayBuffer()`** —— 以 [ArrayBuffer](yhntgbcdexsw/01/)（低级别的二进制数据）形式返回 response。
