...
```js
let socket = new WebSocket("ws://relieved-hai.github.io");
```
...
```js
let socket = new WebSocket("wss://relieved-hai.github.io/article/websocket/demo/hello");
...
出于演示目的，在上面的示例中，运行着一个用 Node.js 写的小型服务器 [server.js](/learn_javascript_asdiasdkc/three/qeiwoquweiz/11/server.js)。它响应为 “Hello from server, John”，然后等待 5 秒，关闭连接。
...
![](./images/websocket-handshake.svg)

这是由 new WebSocket("wss://relieved-hai.github.io/chat") 发出的请求的浏览器 header 示例。

```
GET /chat
Host: relieved-hai.github.io
...
- `Origin` —— 客户端页面的源，例如 `https://relieved-hai.github.io`。WebSocket 对象是原生支持跨源的。没有特殊的 header 或其他限制。旧的服务器无法处理 WebSocket，因此不存在兼容性问题。但是 `Origin` header 很重要，因为它允许服务器决定是否使用 WebSocket 与该网站通信。
...
- `Sec-WebSocket-Protocol: soap, wamp` 表示我们不仅要传输任何数据，还要传输 [SOAP](http://en.wikipedia.org/wiki/SOAP) 或 WAMP（“The WebSocket Application Messaging Protocol”）协议中的数据。WebSocket 子协议已经在 [IANA catalogue](http://www.iana.org/assignments/websocket/websocket.xml) 中注册。
  ...
  ```js
  let socket = new WebSocket("wss://relieved-hai.github.io/chat", ["soap", "wamp"]);
  ```
...
让我们来看一个使用浏览器 WebSocket API 和 Node.js 的 WebSocket 模块 [https://github.com/websockets/ws](https://github.com/websockets/ws) 的聊天示例。我们将主要精力放在客户端上，但是服务端也很简单。
...
```js
let socket = new WebSocket("wss://relieved-hai.github.io/article/websocket/chat/ws");
...
你也可以 [下载它](/learn_javascript_asdiasdkc/three/qeiwoquweiz/11/chat.zip)，然后在本地运行。运行之前请记得安装 [Node.js](https://nodejs.org/en/) 和 `npm install ws`。
