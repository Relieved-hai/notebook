...
`str.search` 方法会查找模式 <code class='pattern'>/love/</code>，然后返回匹配项在字符串中的位置。我们可以猜到，<code class='pattern'>/love/</code> 是最简单的模式。它所做的就是简单的子字符串的查找。
...
所以搜索 <code class='pattern'>/love/</code> 与搜索 `"love"` 是等价的。
...
- regexp – <code class='pattern'>red</code>
- string（我们要搜索的）-- <code class='subject'>blue</code>
- result – <code class='match'>green</code>
:::
...
多行模式（详见章节 [Flag "m" — 多行模式](../../ioprtymcvzq/05/README.md)）。
...
开启完整的 unicode 支持。该修饰符能够修正对于代理对的处理。更详细的内容见章节 [Unicode：修饰符 “u” 和 class \p{...}](../../ioprtymcvzq/03/README.md) 。


**`y`**

粘滞模式（详见 [下一章节](../../ioprtymcvzq/17/README.md) ）
...
2. 使用修饰符 <code class='pattern'>/LOVE/i</code>，在字符串的第 2 个位置上搜索到了 <code class='match'>love</code>。
