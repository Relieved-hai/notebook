...
[FileReader](w3.org/TR/FileAPI/#dfn-filereader) 是一个对象，其唯一目的是从 `Blob`（因此也从 `File`）对象中读取数据。
...
- **`readAsDataURL`** —— 当我们想在 `src` 中使用此数据，并将其用于 `img` 或其他标签时。正如我们在 [Blob](../../yhntgbcdexsw/03/README.md) 一章中所讲的，还有一种用于此的读取文件的替代方案：`URL.createObjectURL(file)`。
...
正如我们在 [Blob](../../yhntgbcdexsw/03/README.md) 一章中所提到的，`FileReader` 不仅可读取文件，还可读取任何 blob。
