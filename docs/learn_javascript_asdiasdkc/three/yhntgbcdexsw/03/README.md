...
```
blob:https://relieved-hai.github.io/notebook/1e67e00e-860d-40a5-89ae-6ab0cbee6273
```
...
[“data-url”](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Basics_of_HTTP/Data_URIs) 的形式为 `data:[<mediatype>][;base64],<data>`。我们可以在任何地方使用这种 url，和使用“常规” url 一样。
...
我们使用内建的 `FileReader` 对象来将 `Blob` 转换为 base64。它可以将 `Blob` 中的数据读取为多种格式。在 [下一章](../../yhntgbcdexsw/04/README.md) 我们将更深入地介绍它。
...
- 1. 使用 [canvas.drawImage](https://developer.mozilla.org/zh-CN/docs/Web/api/CanvasRenderingContext2D/drawImage) 在 `canvas` 上绘制图像（或图像的一部分）。
- 2. 调用 `canvas` 方法 [.toBlob(callback, format, quality)](https://developer.mozilla.org/zh-CN/docs/Web/api/HTMLCanvasElement/toBlob) 创建一个 `Blob`，并在创建完成后使用其运行 `callback`。
...
[XMLHttpRequest](../../qeiwoquweiz/08/README.md) ，[fetch](../../qeiwoquweiz/01/README.md) 等进行 Web 请求的方法可以自然地使用 `Blob`，也可以使用其他类型的二进制数据。
