...
所有这些视图（`Uint8Array`，`Uint32Array` 等）的通用术语是 [TypedArray](https://tc39.es/ecma262/#sec-typedarray-objects) 。它们都享有同一组方法和属性。
...
![](./images/8bit-integer-256.svg)

因此结果是 0。

257 的二进制格式是 `100000001`（9 位），最右边的 8 位会被存储，因此数组中会有 `1`：

![](./images/8bit-integer-257.svg)
...
![](./images/arraybuffer-view-buffersource.svg)
