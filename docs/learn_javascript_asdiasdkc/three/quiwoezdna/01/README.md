...
_`samesite` 会被到 2017 年左右的旧版本浏览器忽略（不兼容）。_
...
获取 cookie 最简短的方式是使用 [正则表达式](../../ioprtymcvzq/01/README.md)。
