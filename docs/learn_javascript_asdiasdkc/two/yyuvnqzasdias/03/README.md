...
<ClientOnly>
  <learn_javascript_asdiasdkc-two-yyuvnqzasdias-03-demo1 />
</ClientOnly>

如果我们想要处理对 `<input>` 的每次更改，那么此事件是最佳选择。

另一方面，`input` 事件不会在那些不涉及值更改的键盘输入或其他行为上触发，例如在输入时按方向键 <kbd>⇦</kbd> <kbd>⇨</kbd>。
...
<ClientOnly>
  <learn_javascript_asdiasdkc-two-yyuvnqzasdias-03-demo2 />
</ClientOnly>
