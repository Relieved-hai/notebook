...
![](./images/document-and-window-coordinates-scrolled.svg)
...
<head></head>
<body>
<button id="abc" onclick="abc()">Get coordinates using button.getBoundingClientRect() for this button</button>
<script>
function abc() {
...
![](./images/coordinates-negative.svg)
...
<html lang="en">
<body>
<button id="coords-show-mark" onclick="qwe()">Button with id=“coords-show-mark”, the message will appear under it</button>
<script>
function qwe() {
  // 用法：
  // 在文档中添加 message 保持 5 秒
