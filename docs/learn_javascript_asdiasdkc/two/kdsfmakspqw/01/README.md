...
<html>
<body>
<input type="button" id="ele" onclick="alert('Before')" value="Click me">
<script>
  ele.onclick = function() { // 覆盖了现有的处理程序
    alert('After'); // 只会显示此内容
  };
</script>
</body>
...
<html>
<body>
<input id="elemDemo" type="button" value="Click me"/>
<script>
  function handler1() {
    alert('Thanks!');
  }
  ...
  const elem = document.getElementById('elemDemo')
  elem.onclick = () => alert("Hello");
  elem.addEventListener("click", handler1); // Thanks!
  elem.addEventListener("click", handler2); // Thanks again!
</script>
