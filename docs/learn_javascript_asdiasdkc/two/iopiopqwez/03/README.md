...
在 [创建自定义事件](../../kdsfmakspqw/05/README.md) 一章中，我们看到过这样一个例子：自定义事件 `menu-open` 被在 `setTimeout` 中分派（dispatched），所以它在 `click` 事件被处理完成之后发生。
...
除了本章中所讲的 **宏任务（macrotask）** 外，还有在 [微任务（Microtask）](../../../one/isjndfafsdqwe/07/README.md) 一章中提到的 **微任务（microtask）**。
