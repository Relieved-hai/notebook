...
对于我们自己的脚本，可以使用 [JavaScript module](../../../one/mknalsweqw/01/README.md) ，但是它们并未被广泛应用于第三方库。
:::
...
`onload/onerror` 处理程序仅跟踪加载过程本身。

它们不会跟踪处理和执行已加载脚本的错误。要“捕获”脚本中的错误，您需要使用 `window.onerror` 全局处理程序。
:::
...
<script src="https://relieved-hai.github.com/notebook/article/onload-onerror/crossorigin/error.js"></script>
```
...
你可以在 [Fetch：跨源请求](../../../three/qeiwoquweiz/05/README.md) 一章中了解有关跨源访问的更多信息。这一章描述了用于网络请求的 `fetch` 方法，但策略是完全相同的。

诸如 “cookie” 之类的内容超出了本章的范围，但你可以在 [Cookie，document.cookie](../../../three/quiwoezdna/01/README.md) 一章学习它们。
:::
