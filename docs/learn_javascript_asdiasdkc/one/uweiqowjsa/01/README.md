...
2. 否则，我们可以用 `x * pow(x, n - 1)` 表示 `pow(x, n)`。在数学里，可能会写为 `x^n = x * x^(n-1)`。这叫做 **一个递归步骤**：我们将任务转化为更简单的行为（`x` 的乘法）和更简单的同类任务的调用（带有更小的 `n` 的 `pow` 运算）。接下来的步骤将其进一步简化，直到 `n` 达到 `1`。

我们也可以说 `pow` **递归地调用自身** 直到 `n == 1`。

![](./images/recursion-pow.svg)
...
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 3, at line 1 }' call='call: pow(2, 3)'/>
...
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 3, at line 5 }' call='call: pow(2, 3)'/>
...
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 2, at line 1 }' call='call: pow(2, 2)'/>
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 3, at line 5 }' call='call: pow(2, 3)' bold='none'/>
...
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 1, at line 1 }' call='call: pow(2, 1)'/>
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 2, at line 5 }' call='call: pow(2, 2)' bold='none'/>
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 3, at line 5 }' call='call: pow(2, 3)' bold='none'/>
...
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 2, at line 5 }' call='call: pow(2, 2)'/>
- <learn_javascript_asdiasdkc-one-uweiqowjsa-01-context context='Context: { x: 2, n: 3, at line 5 }' call='call: pow(2, 3)' bold='none'/>
...
![](./images/recursive-salaries.svg)

我们可以很容易地看到其原理：对于对象 `{...}` 会生成子调用，而数组 `[...]` 是递归树的“叶子”，它们会立即给出结果。

请注意，该代码使用了我们之前讲过的智能特性（smart features）：

- 在 [数组方法](../../wergfhgfhk/05/README.md) 中我们介绍过的数组求和方法 `arr.reduce`。
...
![](./images/linked-list-remove-1.svg)
...
- [递归定义](http://en.wikipedia.org/wiki/Recursive_data_type) 的数据结构是指可以使用自身来定义的数据结构。
