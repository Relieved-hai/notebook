...
让我们拿 [Promise 链](../../isjndfafsdqwe/03/README.md) 那一章的 `showAvatar()` 例子，并将其改写成 `async/await` 的形式：
...
如果我们忘了在这添加 `.catch`，那么我们就会得到一个未处理的 promise error（可以在控制台中查看）。我们可以使用在 [使用 promise 进行错误处理](../../isjndfafsdqwe/04/README.md) 一章中所讲的全局事件处理程序 `unhandledrejection` 来捕获这类 error。
