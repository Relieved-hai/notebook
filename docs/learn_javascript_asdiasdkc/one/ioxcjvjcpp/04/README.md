...
在非严格模式的情况下，`this` 将会是 **全局对象**（浏览器中的 window，我们稍后会在 [全局对象](../../uweiqowjsa/05/README.md) 中见到它）。这是一个历史行为，`"use strict"` 已经将其修复了。
...
在 JavaScript 中，`this` 是“自由”的，它的值是在 **调用时计算出来的**，它的值并不取决于方法声明的位置，而是取决于在 **“点符号前”** 的是什么对象。
...
这是箭头函数的一个特性，当我们 **并不想要一个独立的 `this`**，反而想从外部上下文中获取时，它很有用。在后面的 [深入理解箭头函数](../../uweiqowjsa/11/README.md) 一章中，我们将深入介绍箭头函数。
