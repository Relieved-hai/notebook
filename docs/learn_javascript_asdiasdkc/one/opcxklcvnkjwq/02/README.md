...
如果有任何不清楚的，你可以阅读 [Iterable object（可迭代对象）](../../wergfhgfhk/06/README.md) 一章，其中详细讲解了关于常规迭代器（iterator）的所有内容。
...
简单起见，这里省略了一些解释，即 generator 是“生成（yield）值的函数”。关于此的详细说明请见 [Generator](../../opcxklcvnkjwq/01/README.md) 一章。
...
如果你想了解更多详细内容，请阅读 [Generator](../../opcxklcvnkjwq/01/README.md) 一章。
...
- 1. 我们使用浏览器的 [fetch](../../../three/qeiwoquweiz/01/README.md) 方法来下载 commit。
...
- 3. 我们应该从响应（response）的 `Link` header 中获取前往下一页的 URL。它有一个特殊的格式，所以我们对它使用正则表达式（我们将在 [正则表达式](../../../three/ioprtymcvzq/01/README.md) 一章中学习它）。
