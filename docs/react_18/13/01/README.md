...
> 按位与运算符对两个数的二进制表示进行按位操作，只有两个相应的位都为 1 时，结果的那一位才为 1。
...
> 按位或运算符对两个数的二进制表示进行按位操作，只要两个相应的位中有任何一个为 1，结果的那一位就为 1。
...
> 取反操作指的是将数字的每个位（0 或 1）翻转，即 0 变成 1，1 变成 0。
...
> 按位异或运算符对两个数的二进制表示进行按位操作，当两个相应的位不相同时，结果的那一位为 1；当两个相应的位相同时，结果的那一位为 0。
